...
*/
_.first = function(array, position){
    var firstArray = [];
    var emptArray = [];
    if(Array.isArray(array)){
            if(isNaN(position) || position === undefined){
                return array[0];
            } if(position > 0){
                for(let i = 0; i < position; i++){
                    if(array.length < position){
                        return array;
                    } else firstArray.push(array[i]);
             } return firstArray;
            } else return emptArray = [];
    } else return emptArray = [];
};
...
_.last = function(array, number){
  var startIndex = array.length - number;
  var lastArray = [];
    if(Array.isArray(array)){
...
_.partition = function(array, funct){
   ...
   var subFalse = [];
    for(var i=0;i<array.length;i++){
        if(funct(array[i], i, array) === true){
            subTrue.push(array[i]);
        } else if(funct(array[i],i,array) === false){
            subFalse.push(array[i]);
        }
    } 
    finalArr.push(subTrue);
    finalArr.push(subFalse);
    return finalArr;
};
...
_.unique = function(array){
   ...
   _.each(array, function(item, index, array){
       // if indexOf item in uniArray is nonexistant(-1), push item into uniArray.
       if(_.indexOf(uniArray, item) === -1){
           uniArray.push(item);
       }
   });
   //return the first occurance of all items in uniArray
  return uniArray;    
}
...
* Examples:
*   _.map([1,2,3,4], function(e, key, collection){return e * 2}) -> [2,4,6,8]
*/
_.map = function(collection, getNewValue){
    var newArray = [];
    _.each(collection, function(item, index, collection){
        newArray.push(getNewValue(item, index, collection));
    });
...
_.pluck = function(array, property){
    ...
    return _.map(array, function(item, index, array){
...
_.every = function(collection, funct){
    ...
    _.each(collection, function(item, index, collection){
        ...
        else if(funct(item, index, collection) === false ){
            allMatching = false;
        }
    });
    return allMatching;
...
*/
_.some = function(collection, testFunction){
   var finalReturn = false;
    if(testFunction === undefined){
       testFunction = _.identity;
    }
    _.each(collection, function(item, index, array) {
        if(testFunction(item, index, array)){
            finalReturn = true;
        }
    });
    return finalReturn;
...
_.reduce = function(array, iterator, seed) {
    ...
    if(seed === undefined){
        ...
        _.each(array, function(element, index, collection) {
            index = index + 1;
            prevResult = iterator(prevResult, element, index, collection);
        });
      }else {
          prevResult = seed;
        _.each(array, function(item, index, collection){
            prevResult = iterator(prevResult, item, index,  collection);
        });
    }
